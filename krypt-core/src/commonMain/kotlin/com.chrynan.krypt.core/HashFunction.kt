 ...
 * of type [Input] and outputs a value of type [Result], which contains or is a fixed size hash value
 ...
 */
interface HashFunction<Input, Result> {

    /**
     * Performs the hash with the provided [source] value and returns a [Result].
     */
    suspend operator fun invoke(source: Input): Result
 ...
 */
suspend fun <Input, Result> HashFunction<Input, Result>.hash(source: Input): Result = invoke(source = source)
